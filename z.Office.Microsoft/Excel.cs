...
namespace z.Office.Microsoft
{
    ...
    public class Excel : IDisposable
    {
        ...
        public Excel(string Filename, bool NewFile = true)
        {
            //LoadAssembly();
        ...
        public Excel(Stream fs, bool NewFormat)
        {
        ...
        /// <summary>
        /// LJ 20140128
        /// I just know that i will hide all dll to reference as one
        /// 
        /// Steps:
        ///  Reference DLL -> Properties -> Copy Local -> false
        ///  Add File.DLL -> Properties -> Build Action - > Embedded Resource
        /// </summary>
        void LoadAssembly()
        {
            string NameSpce = "z.Office.Microsoft";
            List<string> Assemblies = new List<string>();
            Assemblies.Add("NPOI.dll");
            Assemblies.Add("NPOI.OOXML.dll");
            Assemblies.Add("NPOI.OpenXml4Net.dll");
            Assemblies.Add("NPOI.OpenXmlFormats.dll");

            foreach (string ress in Assemblies)
            {
                AssemblyResolver.Load(string.Format("{0}.{1}", NameSpce, ress), ress);
            }
        }

        static Assembly CurrentDomain_AssemblyResolve(object sender, ResolveEventArgs args)
        {
            return AssemblyResolver.Get(args.Name);
        }
