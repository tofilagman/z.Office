...
namespace z.Office.Open
{
    public sealed class Calc : IDisposable
    {
        ...
        public Calc()
        {
            this.templateFile = new ZipFile();
        }

        private ZipFile templateFile;
        private XmlDocument contentXml;
        private XmlNamespaceManager nmsManager;
        ...
        {
            this.templateFile = this.GetZipFile(inputFilePath);

            // Get content.xml file
            this.contentXml = this.GetContentXmlFile(templateFile);

            // Initialize XmlNamespaceManager
            this.nmsManager = this.InitializeXmlNamespaceManager(contentXml);
        ...
        private void GetRow(XmlNode rowNode, DataTable sheet, XmlNamespaceManager nmsManager, ref int rowIndex)
        {
            ...
            {
                //while ((sheet.Rows.Count + 1) < rowIndex)
        ...
        private void GetCell(XmlNode cellNode, DataRow row, XmlNamespaceManager nmsManager, ref int cellIndex)
        {
            ...
            if (cellRepeated == null)
            {
                ...
                while (sheet.Columns.Count <= cellIndex)
                    sheet.Columns.Add(cellNode.InnerText, typeof(object)); //LJ 20130627
        ...
        {
            // LJ 20130913, Read the resource file
            this.templateFile = this.GetZipFile(new MemoryStream(z.Office.Open.Properties.Resources.template));
        ...
        /// <param name="odsFile">DataSet that represent .ods file.</param>
        /// <param name="outputFilePath">The name of the file to save to.</param>
        public void WriteOdsFile(DataTable odsFile, string outputFilePath)
        {
            // LJ 20130913, Read the resource file
            this.templateFile = this.GetZipFile(new MemoryStream(z.Office.Open.Properties.Resources.template));
            ...
            this.SaveSheet(odsFile, sheetsRootNode);
